...
def find_matches_nhl():
    current_date = datetime.now().strftime("%Y-%m-%d")
    try:
        df1 = pd.read_csv(f'odds-lines-data/betonline/nhl/nhl_betonline_odds_{current_date}.csv')
...
def find_matches_mlb():
    ...
    try:
        ...
        exit()
    matching_lines = []
    properties = ["PitcherStrikeouts", "HitsAllowed", "RunsAllowed", "TotalBases", "Hits", "Runs", "RBIs", "HitsRunsRBIs", "Singles", "Doubles", "StolenBases",  "BatterWalks", "BatterStrikeouts"] # 'Goals', 'Assists', etc. could be added
    for prop in properties:
        prop_line_betonline = f'{prop}_Line' # 'Shots_Line', 'Points_Line', etc.
        # Iterate over each row in the betonline DataFrame
        for index, row1 in df1.iterrows():
